...
apiClient.interceptors.request.use(
  (config) => {
    ...
    // 如果请求数据是 FormData（例如文件上传），不要使用自定义的 requestTransform
    // 让浏览器设置 `Content-Type: multipart/form-data; boundary=...`
    if (config.data instanceof FormData) {
      config.transformRequest = [];
      // 删除默认的 urlencoded 内容类型，让浏览器自动设置带 boundary 的 multipart 类型
      if (config.headers) {
        // 某些 axios 版本会将 headers 结构为常规对象
        // 删除 Content-Type 以便浏览器自动添加正确的 multipart/form-data
